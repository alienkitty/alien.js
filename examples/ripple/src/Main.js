...
class TitleTexture extends Component {
    ...
    constructor() {
        ...
        function initCanvas() {
            canvas = self.initClass(Canvas, 1920, 1080, true, true);
            self.canvas = canvas;
        ...
        this.update = () => {
            ...
            text = CanvasFont.createText(canvas, 1920, 80, 'Ripple'.toUpperCase(), '200 66px Oswald', Config.UI_COLOR, {
                lineHeight: 80,
                letterSpacing: 0,
                textAlign: 'center'
            });
            const baseline = (1080 - text.totalHeight + 124) / 2;
            text.y = baseline;
            canvas.add(text);
            canvas.render();
